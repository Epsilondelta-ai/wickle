...
**開発状況:** モデルとツールの順次呼び出し、システム入力の分離、結果の保存、イベントの再取得、キャンセル、保存済みの承認・入力・外部作用確認待ちからの再開に対応しています。同じ再開コマンドは重複して処理せず、外部作用の確定には Host による検証を必要とします。一般的な実行中断からの復旧と候補結果の検証は、まだ未対応です。[エージェントの実行](docs/agents.md) · [データ契約](docs/contracts.md)。
