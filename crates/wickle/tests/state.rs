...
fn event(run: &Id, session: &Id, owner: &Scope, seq: u64, payload: RunEventPayload) -> RunEvent {
    RunEvent {
...
fn prepared(snapshot: &RunSnapshot, lease: RunLease, now: i64) -> CommitInput {
...
fn finished(snapshot: &RunSnapshot, lease: RunLease, now: i64) -> CommitInput {
    let mut next = snapshot.clone();
    next.revision += 1;
