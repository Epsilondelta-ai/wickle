...
// These process fixtures deliberately use short initial leases; avoid testing
// scheduler contention between independent fixtures in the same binary.
...
fn a_replacement_process_recovers_an_interrupted_model_with_a_new_charged_attempt() {
    ...
    // The original process has exited without releasing its lease.
...
fn recovery_worker() {
    ...
    tokio::runtime::Builder::new_current_thread()
        ...
        .block_on(async {
            ...
            bindings.clock = Arc::new(SystemClock::default());
...
fn run_tool_worker(directory: &Path, mode: &str) {
    ...
    tokio::runtime::Builder::new_current_thread()
        ...
        .block_on(async {
            ...
            bindings.ids = Arc::new(RandomIdSource);
            if !interrupt {
                // The terminating worker uses the memory fixture's short lease;
                // its replacement exercises the library's normal durable-run settings.
            ...
            if interrupt {
                ...
                let _ = handle.outcome(&context()).await;
                panic!("write did not terminate process");
            }
            let run_id = id(&std::fs::read_to_string(directory.join("run")).unwrap());
            let before = store.load(&scope(), &run_id).await.unwrap();
