...
옵션을 비워두면 전송하지 않습니다. 선택한 모델·API의 schema가 허용하지 않는 값이나 조합은 명시적인 오류로 처리하며, 조용히 무시하거나 다른 수준으로 바꾸지 않습니다. 논리 옵션을 실제 API 필드로 변환하는 부분은 해당 어댑터의 책임이며 실제 SDK 연결은 아직 구현 전입니다.
