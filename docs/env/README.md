...
이 문서는 연결 설정을 준비하기 위한 규약입니다. 실제 어댑터와 모델별 테스트는 구현 중이며, 모델 등록·버전 확인과 실제 호출 결과를 별도로 기록합니다.
...
`.env`와 `.env.*`는 Git에서 제외됩니다. 실제 키나 credential JSON을 문서·커밋에 넣지 않습니다. Core 라이브러리는 `.env`를 직접 읽지 않고, Host나 테스트 실행기가 설정을 읽어 어댑터를 구성합니다.
