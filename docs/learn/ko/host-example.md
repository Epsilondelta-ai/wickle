...
[실행 방법과 해설](walkthrough.md) · [목차](README.md)

0.1.0의 `tests/support/agent_consumer.rs` 완성본이다. 생략 없이 별도 Host의 `src/main.rs`로 사용할 수 있다. Catalog, route, policy, inspector, model, estimator를 읽은 뒤 main의 객체 조립을 따라간다. fake policy·고정 시계·metadata echo는 테스트를 위한 것이며 운영 인증/검사 구현은 아니다.
